        ...
        if(stringData) {
            ...
            const data = request.data.swapLogs.map((item: ConversionData) => {
                ...
                return {
                    ...
                    ops: smartContractOps.length > 0 ? smartContractOps.map((op: SmartContractOps) => ({
                        id: op?.id,
                        feedback: {
                            ...op?.feedback,
                            createdAt: item?.createdAt 
                    ...
                    tx: smartContractOps.length > 0 ? smartContractOps.reduce((acc: string, op: SmartContractOps) => {
                        acc = op.tx;
                        return acc;
                    }, '') : null,
                    feedback: smartContractOps.length > 0 ? smartContractOps.reduce((acc: Feedback, op: SmartContractOps) => {
                        acc = op?.feedback;
                        return acc;
                    }, {} as Feedback) : null,
